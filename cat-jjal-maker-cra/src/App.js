...
const MainCard = ({ img, onHeartClick, favoritesStatus }) => {
  console.log("favoritesStatus", favoritesStatus)

  const heartIcon = favoritesStatus ? "💖" : "🤍";
...
const App = () => {
  ...
  const [favoritesStatus, setFavoritesStatus] = React.useState(favorites.includes(mainCat));
  React.useEffect(() => {
    setInitCat();
  }, [])
  ...
  return (
    <div>
      <Title>{cntCat}고양이 가라사대2</Title>
      <Form updateMainCat={updateMainCat} />
      <MainCard img={mainCat} onHeartClick={handlerHeartCl} favoritesStatus={favoritesStatus} />
